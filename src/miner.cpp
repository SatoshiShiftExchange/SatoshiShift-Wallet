...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SatoshishiftMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("SatoshishiftMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("SatoshishiftMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("satoshishift-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running SatoshishiftMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
