...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::SHFT, nValue);
...
{
    std::string genesisblockhash = "000009400d98c132d0dacb793b5daa61b0a9784e00596a7170506c42d3c1586c";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (satoshishift.conf).");
            QMessageBox::warning(this, "Satoshishift Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
